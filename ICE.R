...
for(j in 1:length(group))###calculate the entrophy for each sub-group计算每个子组的熵
{
  ...
  for(k in 1:nrow(sub_group))
  {
    ...
    CEi = CEi*0 + (- log(as.numeric(as.numeric(frequency[row.names(sub_group)[k],2])))*as.numeric(frequency[row.names(sub_group)[k],2]))
...
driver_gene = read.csv('Census.csv')
#driver_gene = read.csv('NCG6_cancergenes.csv')

length(total_gene1[,1][which( total_gene1[,1][1:200] %in%driver_gene$Gene.Symbol)]) 
pre = c()
for (r in 1:200)
{
  pre=rbind(pre,cbind(r,length(total_gene1[,1][which( total_gene1[,1][1:r] %in%driver_gene$Gene.Symbol)])/r))
}
pre[,2][50]
pre[,2][100]
pre[,2][200]
...
for (r in 1:200)
{
  rec=rbind(rec,cbind(r,length(total_gene1[,1][which( total_gene1[,1][1:r] %in%driver_gene$Gene.Symbol )])/301))
}
rec[,2][50]
rec[,2][100]
rec[,2][200]

#feb = CER[which( CER[,1] %in% total_gene1[,1][1:30] ),]

#write.table (total_gene1, file ="ICE.csv",row.names = FALSE, col.names =TRUE, quote =FALSE)
#write.table (pre, file ="pre.csv",row.names = FALSE, col.names =TRUE, quote =FALSE)
#write.table (feb, file ="Lung_fenbu.csv",row.names = FALSE, col.names =TRUE, quote =FALSE)

#write.table (finalresult1, file ="Lung.csv",row.names = FALSE, col.names =TRUE, quote =FALSE)
